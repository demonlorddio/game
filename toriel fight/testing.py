...
attack_phase = False
attack_start_time = 0  # Attack phase duration in seconds
# Colors
black = (0, 0, 0)
white = (255, 255, 255)
red = (255, 0, 0)
...
escape=pygame.mixer.Sound('escaped.mp3')
...
def draw_text_animated(surface, font, text, x, y, delay=50):
    ...
    elif toriel_hp<=0 and toriel_hp>-100:
        screen.blit(toriel_dying_image, (width // 2 - toriel_image.get_width() // 2, 80))
    elif toriel_hp>0:
...
def display_dialogues(dialogues):
    for dialogue in dialogues:
        ...
        for i, text in enumerate(dialogue):
            ...
            pygame.display.flip()
            pygame.time.wait(10)  # Wait between lines
        pygame.time.wait(1500)  # Wait between dialogues
    pygame.display.flip()
                ...
                if event.key == pygame.K_UP:
                    ...
                    if options[selected_option] == "SPARE":
                        ...
                        for _ in range(30):  # Adjust the range for animation duration
                            ...
                            heart_cursor_rect.width=50
                            heart_cursor_rect.height=500
                            heart_cursor_rect.x -= 2  # Falling speed
                            screen.fill(black)
            ...
            if "angle" in bullet:
                ...
                if heart_rect.colliderect(bullet["rect"]):
                    if current_spare_index>11:
                        bullet['rect'].y-=25 or -25
                        bullet['rect'].x-=25 or -25
                    else:
                        hp -= 1
                        damage_sound.play()
...
def game_over():
    ...
    player_heart_break(screen, heart_image, broken_heart_image, heart_rect)
    ...
    # Game Over Screen
    game_over_screen()
def player_heart_break(screen, heart_image, broken_heart_image, heart_rect):
    # Display the broken heart image for a moment
    screen.fill((0, 0, 0))
    screen.blit(broken_heart_image, heart_rect.topleft)
    pygame.display.flip()
    pygame.time.delay(1000)  # Display broken heart for 1 second

    # Break the heart into six pieces and fall
    fall_pieces = []
    piece_width = broken_heart_image.get_width() // 2
    piece_height = broken_heart_image.get_height() // 3
    for i in range(3):
        for j in range(2):
            fall_pieces.append({
                "surface": broken_heart_image.subsurface(pygame.Rect(j * piece_width, i * piece_height, piece_width, piece_height)),
                "x": heart_rect.x + j * piece_width,
                "y": heart_rect.y + i * piece_height,
                "vx": random.uniform(-1, 1),
                "vy": random.uniform(1, 20),  # Adjust falling speed here
                "rotation": random.uniform(-5, 5),
                "rotation_speed": random.uniform(-1, 1)
            })
    
    for _ in range(255, 0, -5):
        screen.fill((0, 0, 0))
        for particle in fall_pieces:
            particle["x"] += particle["vx"]
            particle["y"] += particle["vy"]
            particle["rotation"] += particle["rotation_speed"]
            particle_surface = pygame.transform.rotate(pygame.transform.scale(particle["surface"], (piece_width, piece_height)), particle["rotation"])
            screen.blit(particle_surface, (int(particle["x"]), int(particle["y"])))
...
def final_decision():
    ...
    while running:
        ...
        if choice == "FIGHT":
            ...
            for _ in range(30):
                offset_xa = random.randint(-15, 15)
                offset_xb = random.randint(-50, 50)
                pygame.mixer.music.pause()
                
                screen.fill(black)
                screen.blit(toriel_betrayed_image, (width // 2 - toriel_defeat_image.get_width() // 2+offset_xb , 50 ))
...
clock = pygame.time.Clock()
#create_bullets(10)
create_bullets(random.randint(10,15))
#create_green_bullets(0)
create_green_bullets(random.randint(0,5))
...
def main_game_loop():
    ...
    #current_spare_index=11 #change for testing purpose
    while running:

        
        dodging_time = random.randint(5,10)  # Example dodge time in seconds
        if hp <= 0:
            game_over()
            break
        txt=random.choice(['* Toriel blocks the way!','* Toriel looks through you.','* Toriel takes a deep breath.','* Toriel prepares a magical attack.','* Toriel is acting aloof'])
