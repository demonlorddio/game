...
undyne_hurt=pygame.transform.scale((pygame.image.load('undyne hurt.png')),(173,234))
current_frame = 0
...
class Arrow(pygame.sprite.Sprite):
    def __init__(self, direction):
        ...
        self.speed = 1#random.randint(2,3)
...
def shake_screen(damage):
    ...
    for _ in range(10):
        ...
        draw_health_bar(hp, max_hp, 550, height - 60, YELLOW, screen)
        screen.fill(BLACK)
        screen.blit(undyne_hurt, (width/2-100+offset_xa, 20))  # Apply shake to UNDYNE image
        draw_health_bar(undyne_hp, undyne_max_hp, 550+offset_x, height - 550, RED, screen)

        # Draw damage number during the shake
        draw_damage_number(screen, int(damage), 675+offset_xa , 180+offset_y)
        pygame.display.flip()
        pygame.time.wait(50)
...
def items_phase():
    global hp, undyne_hp,current_frame, last_update
...
def display_item_message(item, heal_amount):
    if True:
        ...
        screen.blit(undyne_hurt, (width // 2 - undyne_hurt.get_width() // 2, 50))
        pygame.display.flip()
